...
The Greentic CLI provides commands for developing, testing, and deploying digital workers.
...
Initialize your Greentic environment:
...
- Generates a `GREENTIC_TOKEN` for accessing the store
...
```bash
greentic secrets add <KEY> <value>
```

**Example:**
```bash
greentic secrets add TELEGRAM_TOKEN your_bot_token
greentic secrets add WEATHERAPI_KEY your_api_key
```
...
Manage flows: create, validate, deploy, and run.

### pull

Pull a flow from the Greentic store:

```bash
greentic flow pull <flow_name>.ygtc
```
...
### Flow Authoring Commands

The following commands edit YGTc v2 flows in-place and maintain a resolve sidecar (`<flow>.ygtc.resolve.json`) that tracks component bindings.
...
|----------|-------------|
| `GREENTIC_TOKEN` | Authentication token for the Greentic store (created by `greentic init`) |
| `GREENTIC_CONFIG` | Config file path (`~/.greentic/config.toml`) |
| `RUST_LOG` | Log level (e.g., `greentic=info`) |
