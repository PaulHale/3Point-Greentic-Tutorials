...
<Terminal
  client:visible
  lines={[
    { type: 'command', content: 'greentic-flow add-step --flow flows/echo.ygtc --node-id greet --operation templating.handlebars --config-file greet-config.json --routing-next echo' },
